...
if (!getPluginStatusActive('comments')) {
	msg(['type' => 'error', 'text' => 'Для работы Подписка на комментарии требуется ниличия плагина <strong>comments</strong>.']);
	return print_msg( 'warning', 'Подписка на комментарии', 'Внимание!<br>Плагин <strong>comments</strong> не был установлен. Для использования этого дополнения, необходимо установить плагин <a href="?mod=extra-config&plugin=comments">comments</a>', '?mod=extras' );
}
...
function show_list_subscribe() {
	...
	global $tpl, $mysql, $lang, $config, $breadcrumb;
	$tpath = locatePluginTemplates(array('config/main', 'config/list_subscribe', 'config/list_entries'), 'subscribe_comments', 1);
	
	$breadcrumb = breadcrumb('<i class="fa fa-address-card btn-position"></i><span class="text-semibold">Список подписчиков</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=subscribe_comments' => '<i class="fa fa-address-card btn-position"></i>Подписка на комментарии', 'Список подписчиков' ) );
	...
	foreach ($mysql->select('SELECT *, c.id as nid from ' . prefix . '_subscribe_comments AS c left join ' . prefix . '_news AS s on c.news_id=s.id ORDER BY c.id DESC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		$url = "?mod=news&amp;action=edit&amp;id=" . $row['news_id'];
		$gvars['vars'] = array(
			'id'        => $row['nid'],
			'page_name' => $row['title'],
			'page_url'  => $url,
...
function modify() {
	...
	if (isset($del)) {
		...
		return print_msg( 'delete', 'Подписка на комментарии', 'Пиьсма с ID '.$id.' удалены', 'javascript:history.go(-1)' );
	}
	if (isset($del_post)) {
...
function main() {
	...
	global $tpl, $mysql, $config, $template, $cron, $breadcrumb, $lang;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-address-card btn-position"></i><span class="text-semibold">Подписка на комментарии</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-address-card btn-position"></i>Подписка на комментарии' ) );
	...
	if (isset($_REQUEST['submit'])) {
		...
		return print_msg( 'info', 'Подписка на комментарии', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
