...
function add_field() {
	...
	global $mysql, $twig, $lang, $breadcrumb;
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Добавить' ) );
	$tVars['field'] = "";
...
function insert_field() {
	...
	return print_msg( 'success', 'Гостевая книга', 'Поле успешно добавлено!', 'javascript:history.go(-1)' );
...
function update_field() {
	...
	return print_msg( 'update', 'Гостевая книга', 'Поле '.$name.' успешно отредактировано!', array('?mod=extra-config&plugin=guestbook&action=edit_field&id='.$id => 'Редактировать еще', '?mod=extra-config&plugin=guestbook&action=manage_fields' => 'Вернуться назад' ) );
...
function drop_field() {
	...
	return print_msg( 'delete', 'Гостевая книга', 'Файлы были успешно удалены!', 'javascript:history.go(-1)' );
}
...
function manage_fields() {
	...
	global $mysql, $twig, $lang, $breadcrumb;
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Управление полями' ) );
...
function show_options() {
	...
	global $tpl, $mysql, $lang, $breadcrumb, $twig;
	$tpath = locatePluginTemplates(array('config/main', 'config/settings'), 'guestbook', 1);
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Настройки' ) );
	if (isset($_REQUEST['submit'])) {
		...
		return print_msg( 'info', 'Гостевая книга', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
	}
...
function show_messages() {
	...
	global $tpl, $mysql, $lang, $breadcrumb, $twig, $config, $PHP_SELF;
	$tpath = locatePluginTemplates(array('config/main', 'config/messages_list'), 'guestbook', 1);
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Список записей' ) );
	$tVars = array();
	...
	$tVars = array(
		'pagesss'   => generateAdminPagelist(array('current' => $pageNo, 'count' => $countPages, 'url' => '?mod=extra-config&plugin=guestbook&action=show_messages&page=%page%')),
...
function delete_social() {
	...
	print_msg( 'delete', 'Гостевая книга', 'Выбранная соц.сеть была успешна удалена!', 'javascript:history.go(-1)' );
	return $id;
}
...
function delete_message() {
	...
	msg(array("text" => $lang['gbconfig']['msgo_deleted_one']));
	return print_msg( 'delete', 'Гостевая книга', 'Выбранное сообщение была успешно удалена!', 'javascript:history.go(-1)' );
}
...
function edit_message() {
	...
	if (!empty($id)) {
		...
		if (isset($_REQUEST['submit'])) {
			...
			if (!count($errors)) {
				...
				return print_msg( 'update', 'Гостевая книга', 'Запись сообщения '.$message.' успешно изменено!', array('?mod=extra-config&plugin=guestbook&action=edit_message&id='.$id => 'Редактировать еще', '?mod=extra-config&plugin=guestbook&action=manage_fields' => 'Вернуться назад' ) );

			} else {
				msg(array("type" => "error", "text" => implode($errors)));
				return print_msg( 'warning', 'Гостевая книга', 'Внимание!<br>Произошла ошибка ввода - вывода.', 'javascript:history.go(-1)' );
			}
		}
		// output social data
		...
		return print_msg( 'warning', 'Гостевая книга', 'Внимание!<br>Произошла ошибка ID не передан.', '?mod=extra-config&plugin=guestbook' );
	}
	...
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Редактирование записи - '.$id.'' ) );
	$xg = $twig->loadTemplate($tpath['config/main'] . 'config/main.tpl');
	$tVars = array(
		'global' => "Редактирование записи",
		'panel' => "Редактирование",
		'entries' => $xt->render($tVars),
	);
	print $xg->render($tVars);
}
...
function modify() {
	...
	return print_msg( 'success', 'Гостевая книга', 'Новая запись успешно добавлено.', '?mod=extra-config&plugin=guestbook&action=show_messages' );
...
function social_config() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-leanpub btn-position"></i><span class="text-semibold">Гостевая книга</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=guestbook' => '<i class="fa fa-leanpub btn-position"></i>Гостевая книга',  'Привязка соц.сетей' ) );
	if (isset($_REQUEST['submit'])) {
