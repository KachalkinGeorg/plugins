...
function main() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-file-image-o btn-position"></i><span class="text-semibold">Gallery Manager</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=gmanager' => '<i class="fa fa-file-image-o btn-position"></i>Gallery Manager',  ''.$lang['gmanager:ULIB_main'].'' ) );
	...
	$tvars['vars']['panel'] = $lang['gmanager:ULIB_main'];
	$tvars['vars']['active1'] = 'active';
...
function general_submit() {
	...
	return print_msg( 'info', 'Gallery Manager', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
}

function showlist() {

	global $tpl, $lang, $breadcrumb, $mysql;
	$tpath = locatePluginTemplates(array('conf.main', 'conf.list', 'conf.list.row'), 'gmanager', 1);
	$breadcrumb = breadcrumb('<i class="fa fa-file-image-o btn-position"></i><span class="text-semibold">Gallery Manager</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=gmanager' => '<i class="fa fa-file-image-o btn-position"></i>Gallery Manager',  ''.$lang['gmanager:button_list'].'' ) );
	$output = '';
	...
	$tvars['vars']['panel'] = $lang['gmanager:button_list'];
	$tvars['vars']['active2'] = 'active';
	$tpl->template('conf.main', $tpath['conf.main']);
	$tpl->vars('conf.main', $tvars);
	print $tpl->show('conf.main');
}
...
function edit() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-file-image-o btn-position"></i><span class="text-semibold">Gallery Manager</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=gmanager' => '<i class="fa fa-file-image-o btn-position"></i>Gallery Manager',  'Редактирование '.$galery['name'].'' ) );
...
function edit_submit() {
	...
	if ($t_update) {
		...
		return print_msg( 'update', 'Gallery Manager', 'Галерея '.$title.' успешно отредактирована!', array('?mod=extra-config&plugin=gmanager&action=edit&id='.$id => 'Редактировать еще', '?mod=extra-config&plugin=gmanager&action=list' => 'Вернуться назад' ) );
...
function delete() {
	...
	if (isset($_POST['commit'])) {
		if ($_POST['commit'] == 'yes') {
			...
			return print_msg( 'delete', 'Gallery Manager', 'Удаление прошло успешно!', 'javascript:history.go(-1)' );
			if (pluginGetVariable('gmanager', 'if_auto_cash')) clear_cash();
		}
		showlist();
...
function clear_cash() {
	...
	if (($dir = get_plugcache_dir('gmanager'))) {
		...
		return print_msg( 'cache', 'Gallery Manager', ''.$lang['gmanager:info_cash_clear'].'', 'javascript:history.go(-1)' );
	}
}