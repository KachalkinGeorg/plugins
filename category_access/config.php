...
function main() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-folder-o btn-position"></i><span class="text-semibold">Доступ к категориям</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-folder-o btn-position"></i>Главная' ) );
...
function show_list_user() {
	...
	global $tpl, $lang, $breadcrumb, $catz, $catmap;
	$tpath = locatePluginTemplates(array('conf.main', 'conf.list.user', 'conf.list.user.row'), 'category_access', 1);
	$breadcrumb = breadcrumb('<i class="fa fa-folder-o btn-position"></i><span class="text-semibold">Доступ к категориям</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=category_access' => '<i class="fa fa-folder-o btn-position"></i>Доступ к категориям',  'Список пользователей' ) );
...
function show_list_category() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-folder-o btn-position"></i><span class="text-semibold">Доступ к категориям</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=category_access' => '<i class="fa fa-folder-o btn-position"></i>Доступ к категориям',  'Список категорий' ) );
...
function add_user() {
	...
	global $tpl, $lang, $breadcrumb, $catz, $catmap, $mysql;
	$users = pluginGetVariable('category_access', 'users');
	$breadcrumb = breadcrumb('<i class="fa fa-folder-o btn-position"></i><span class="text-semibold">Доступ к категориям</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=category_access' => '<i class="fa fa-folder-o btn-position"></i>Доступ к категориям',  'Добавить пользователя' ) );
	$if_add = true;
	$user = '';
	$category = 0;
	if (isset($_GET['user'])) {
	...
	if (isset($_POST['user']) && isset($_POST['category'])) {
		...
		if (!$if_error) {
			...
			pluginsSaveConfig();
			msg(array('type' => 'info', 'info' => $lang['category_access:info_save_general']));
			print_msg( 'info', 'Доступ к категориям', 'Действия для пользователя назначены!', '?mod=extra-config&plugin=category_access&action=list_user' );
			return;
		}
	}
	$category_list = array();
...
function add_category() {
	...
	global $tpl, $lang, $breadcrumb, $catz, $catmap;
	$categorys = pluginGetVariable('category_access', 'categorys');
	$breadcrumb = breadcrumb('<i class="fa fa-folder-o btn-position"></i><span class="text-semibold">Доступ к категориям</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=category_access' => '<i class="fa fa-folder-o btn-position"></i>Доступ к категориям',  'Добавить категорию' ) );
	if (isset($_POST['category']) && is_array($_POST['category'])) {
		...
		pluginsSaveConfig();
		msg(array('type' => 'info', 'info' => $lang['category_access:info_save_general']));
		print_msg( 'info', 'Доступ к категориям', 'Действия для категории назначены!', '?mod=extra-config&plugin=category_access&action=list_category' );
		return;
	}
...
function delete_user() {
	...
	if (isset($_POST['commit'])) {
		...
		print_msg( 'delete', 'Доступ к категориям', 'Выбранный пользователь был успешно удален!', 'javascript:history.go(-1)' );
		return;
	}
...
function delete_category() {
	...
	if (isset($_POST['commit'])) {
		...
		print_msg( 'delete', 'Доступ к категориям', 'Выбранная категория была успешна удалена!', 'javascript:history.go(-1)' );
		return;
	}
