...
class NSchedNewsFilter extends NewsFilter
{
    ...
    public function addNewsForm(&$tvars)
    {
		...
		$tpath = locatePluginTemplates(array('add_news'), 'nsched', pluginGetVariable('nsched', 'localsource'));
        $tvars['extends'][$extends][] = [
			'title' => "Управление публикацией новостей",
			'body' => $twig->render($tpath['add_news'].'/add_news.tpl', $ttvars),
			];
    ...
    public function editNewsForm($newsID, $SQLold, &$tvars)
    {
		...
		$tpath = locatePluginTemplates(array('edit_news'), 'nsched', pluginGetVariable('nsched', 'localsource'));
        $tvars['extends'][$extends][] = [
			'title' => "Управление публикацией новостей",
			'body' => $twig->render($tpath['edit_news'].'/edit_news.tpl', $ttvars),
			];
