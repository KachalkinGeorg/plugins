...
function show_xsyslog() {
	...
	global $tpl, $mysql, $lang, $twig, $confArray, $breadcrumb;
	$tpath = locatePluginTemplates(array('main', 'list_xsyslog', 'list_entries'), 'xsyslog', 1);
	
	$breadcrumb = breadcrumb('<i class="fa fa-info-circle btn-position"></i><span class="text-semibold">Журнал действий пользователей</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-info-circle btn-position"></i>Журнал действий пользователей' ) );
	...
	$tVars = array(
		'pagesss' => generateAdminPagelist(array('current' => $pageNo, 'count' => $countPages, 'url' => '?mod=extra-config&plugin=xsyslog' . ($news_per_page ? '&rpp=' . $news_per_page : '') . ($fAuthorName ? '&an=' . $fAuthorName : '') . ($fStatus ? '&status=' . $fStatus : '') . ($fPlugin ? '&fplugins=' . $fPlugin : '') . ($fItem ? '&fitems=' . $fItem : '') . ($fDateStart ? '&dr1=' . $fDateStart : '') . ($fDateEnd ? '&dr2=' . $fDateEnd : '') . '&page=%page%')),
		'entries' => isset($tEntry) ? $tEntry : ''
	);
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'entries'    => $xt->render($tVars),
		'global'       => 'Журнал пользователей',
		'panel'       => 'Список',
