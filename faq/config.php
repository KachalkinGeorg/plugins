...
function show_add_faq() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-question-circle-o btn-position"></i><span class="text-semibold">Вопросы и ответы</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=faq' => '<i class="fa fa-question-circle-o btn-position"></i>Вопросы и ответы',  'Добавить' ) );
	if (isset($_REQUEST['submit'])) {
		...
		if (empty($error_text)) {
			$mysql->query('INSERT INTO ' . prefix . '_faq (question, answer, active) 
				...
				');
			msg(array("type" => "info", "info" => "Добавлено успешно"));
			return print_msg( 'info', 'Вопросы и ответы', 'Новый вопрос и ответ успешно добавлены!', '?mod=extra-config&plugin=faq' );
...
function show_edit_faq() {
		...
		if (isset($_REQUEST['submit'])) {
			...
			if (empty($error_text)) {
				$mysql->query('UPDATE ' . prefix . '_faq SET 
						...
						WHERE id = ' . intval($id) . ' ');
				//var_dump($_REQUEST);
				msg(array("type" => "info", "info" => "Запись обновлена"));
				return print_msg( 'update', 'Вопросы и ответы', 'Вопрос успешно обнавлен!', '?mod=extra-config&plugin=faq' );
			}
		}
		if (!empty($error_text)) {
			foreach ($error_text as $error) {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-question-circle-o btn-position"></i><span class="text-semibold">Вопросы и ответы</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=faq' => '<i class="fa fa-question-circle-o btn-position"></i>Вопросы и ответы',  ''.$row['answer'].'' ) );
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global' => 'Вопросы и ответы',
		'panel' => 'Редактировать',
		'entries' => $xt->render($tVars),
	);
	print $xg->render($tVars);
...
function modify() {
	...
	return print_msg( 'delete', 'Вопросы и ответы', 'Выбранная запись успешна удалена!', 'javascript:history.go(-1)' );
}
...
function show_faq() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-question-circle-o btn-position"></i><span class="text-semibold">Вопросы и ответы</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=faq' => '<i class="fa fa-question-circle-o btn-position"></i>Вопросы и ответы',  'Список' ) );
	$tVars = array();
	// Records Per Page
	// - Load
	$news_per_page = 10;
	...
	$tVars = array(
		'pagesss'   => generateAdminPagelist(array('current' => $pageNo, 'count' => $countPages, 'url' => '?mod=extra-config&plugin=faq' . ($news_per_page ? '&rpp=' . $news_per_page : '') . '&page=%page%')),
