...
function showlist()
{
	global $tpl, $mysql, $lang, $breadcrumb;
	...
	$breadcrumb = breadcrumb('<i class="fa fa-file-text btn-position"></i><span class="text-semibold">Перенаправление статики</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-file-text btn-position"></i>Перенаправление статики' ) );
	...
	$tvars['vars']['global'] = "Список";
	$tvars['vars']['panel'] = "Главная";
...
function editform()
{
	global $mysql, $tpl, $config, $lang, $breadcrumb;
	...
	if (isset($_REQUEST['code']) && isset($_REQUEST['idstat'])){
		...
		if (!$if_error){
			...
			$ULIB->saveConfig();
			return print_msg( 'update', 'Перенаправление статики', 'Все успешно отредактировано!', '?mod=extra-config&plugin=re_stat' );
		}
	}
	$static_page = $mysql->select('select `id`, `title` from '.prefix.'_static order by `title`, `id`');
	...
	if ($id == -1) {
		$breadcrumb = breadcrumb('<i class="fa fa-file-text btn-position"></i><span class="text-semibold">Перенаправление статики</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-file-text btn-position"></i>Добавить' ) );
	}else{
		$breadcrumb = breadcrumb('<i class="fa fa-file-text btn-position"></i><span class="text-semibold">Перенаправление статики</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', 'Редактировать код - '.$values[$id]['code'].'' ) );
	}
...
function delete()
{
	...
	if (!is_array($values)) {
		...
		return print_msg( 'delete', 'Перенаправление статики', 'Удаление прошло успешно!', 'javascript:history.go(-1)' );
	}
	if (!array_key_exists($id, $values)) {
		msg(array('type' => 'info', 'info' => '<font color="red">Ключ id='.$id.' отсутствует в базе</font>'));
		return print_msg( 'warning', 'Перенаправление статики', 'Внимание!<br>Вы обартились к несуществующем ключу. Данный ключ отсутствует в базед данных.', '?mod=extra-config&plugin=re_stat' );
	}
	...
	pluginSetVariable('re_stat', 'values', $values);
	pluginsSaveConfig();
	return print_msg( 'info', 'Перенаправление статики', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
}
