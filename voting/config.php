...
if ($_REQUEST['action'] == 'newvote') {
	...
	//print "<div class='content'>Новый опрос создан. <a href='$PHP_SELF?mod=extra-config&plugin=voting'>переход к редактированию</a></div>";
	msg(array("type" => "info", "info" => "Новый опрос создан"));
	return print_msg( 'success', 'Voting manager', 'Новый опрос создан!', array('?mod=extra-config&plugin=voting&action=newvote' => 'Добавить еще', '?mod=extra-config&plugin=voting' => 'Вернуться назад' ) );
	...
	if ($row = $mysql->record("select * from " . prefix . "_vote where id = $voteid")) {
		...
		msg(array("type" => "info", "info" => "Опрос удален"));
		return print_msg( 'delete', 'Voting manager', 'Выбранный опрос был успешно удален!', 'javascript:history.go(-1)' );
	} else {
		msg(array("type" => "info", "info" => "Такого опроса не существует."));
		return print_msg( 'warning', 'Voting manager', 'Внимание!<br>Данного опроса не существует! Возможно он был удален ранее.', 'javascript:history.go(-1)' );
	}
	...
	print_msg( 'info', ''.$plugin.'', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
} else {
