...
function general_submit() {
	...
	if (!$if_error) {
		...
		return print_msg( 'info', 'Multy main', $lang['multi_main:info_save_general'], '?mod=extra-config&plugin=multi_main' );
	}
}
...
function main() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-home btn-position"></i><span class="text-semibold">Multy main</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-home btn-position"></i>Multy main' ) );
...
function showlist() {
	...
	global $tpl, $lang, $catz, $breadcrumb;
	
	$breadcrumb = breadcrumb('<i class="fa fa-home btn-position"></i><span class="text-semibold">Multy main</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=multi_main' => '<i class="fa fa-home btn-position"></i>Multy main', ''.$lang['multi_main:button_list'].'' ) );
...
function add() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-home btn-position"></i><span class="text-semibold">Multy main</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=multi_main' => '<i class="fa fa-home btn-position"></i>Multy main', ''.$lang['multi_main:button_add_group'].'' ) );
	...
	if (isset($_GET['cat'])) {
		if (!array_key_exists($_GET['cat'], $category)) {
			...
			return print_msg( 'error', 'Multy main', ''.$lang['multi_main:error_not_exists'].'<br>'.$lang['multi_main:error_val_title'].'', 'javascript:history.go(-1)' );
		}
		$cat = $_GET['cat'];
		$tpll = $category[$cat];
		$if_add = false;
	...
	if (isset($_POST['cat']) && isset($_POST['tpl'])) {
		...
		if (!$cat || !validate($cat)) {
			...
			return print_msg( 'error', 'Multy main', ''.sprintf($lang['multi_main:error_validate'], $lang['multi_main:label_cat']).'<br>'.$lang['multi_main:error_val_title'].'', 'javascript:history.go(-1)' );
			$if_error = true;
		}
		if (!$tpll || !validate($tpll)) {
			...
			return print_msg( 'error', 'Multy main', ''.sprintf($lang['multi_main:error_validate'], $lang['multi_main:label_tpl']).'<br>'.$lang['multi_main:error_val_title'].'', 'javascript:history.go(-1)' );
			$if_error = true;
		}
		if ($if_add && array_key_exists($cat, $category)) {
			...
			return print_msg( 'error', 'Multy main', ''.sprintf($lang['multi_main:error_exists'], $catz[$cat]['name']).'<br>'.$lang['multi_main:error_val_title'].'', 'javascript:history.go(-1)' );
			$if_error = true;
		}
		if (!$if_error) {
			$category[$cat] = $tpll;
			...
			pluginsSaveConfig();
			msg(array('type' => 'info', 'info' => $lang['multi_main:info_save_general']));
			return print_msg( 'info', 'Multy main', ''.$lang['multi_main:info_save_general'].'', '?mod=extra-config&plugin=multi_main&action=list_menu' );
		}
	}
	$cat_list = array();
...
function delete() {
	...
	if (!isset($_REQUEST['cat']) || !array_key_exists($_REQUEST['cat'], $category)) {
		msg(array('type' => 'error', 'info' => $lang['multi_main:error_not_exists'], 'text' => $lang['multi_main:error_val_title']));
		return print_msg( 'error', 'Multy main', ''.$lang['multi_main:error_not_exists'].'<br>'.$lang['multi_main:error_val_title'].'', '?mod=extra-config&plugin=multi_main&action=list_menu' );
	}
	$cat = $_REQUEST['cat'];
	if (isset($_POST['commit'])) {
		...
		return print_msg( 'delete', 'Multy main', ''.$lang['multi_main:info_save_general'].'', '?mod=extra-config&plugin=multi_main&action=list_menu' );
	}
