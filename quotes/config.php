...
function showlist()
{global $mysql, $tpl, $template, $config, $parse, $lang, $breadcrumb;
	$tpath = locatePluginTemplates(array('config/conf.quotes', 'config/conf.quotes.row'), 'quotes', pluginGetVariable('quotes', 'localsource'));

	$breadcrumb = breadcrumb('<i class="fa fa-paragraph btn-position"></i><span class="text-semibold">Цитатник</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'',  '<i class="fa fa-paragraph btn-position"></i> Цитатник' ) );
	...
	$tvars['vars']['pagesss'] = generateAdminPagelist( array('current' => $pageNo, 'count' => $countPages, 'url' => '?mod=extra-config&plugin=quotes&action=showlist'.($_REQUEST['news_per_page']?'&news_per_page='.$news_per_page:'').($_REQUEST['author']?'&author='.$_REQUEST['author']:'').($_REQUEST['sort']?'&sort='.$_REQUEST['sort']:'').($postdate?'&postdate='.$postdate:'').($author?'&author='.$author:'').($status?'&status='.$status:'').'&page=%page%'));
...
function addform()
{global $tpl, $template, $mysql, $userROW, $lang, $breadcrumb;
