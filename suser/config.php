...
function url() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-random btn-position"></i><span class="text-semibold">ЧПУ</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '?mod=extra-config&plugin=suser' => '<i class="fa fa-address-card btn-position"></i>Список пользователей', '<i class="fa fa-random btn-position"></i>ЧПУ' ) );
...
function main() {
	...
	$breadcrumb = breadcrumb('<i class="fa fa-address-card btn-position"></i><span class="text-semibold">Список пользователей</span>', array('?mod=extras' => '<i class="fa fa-puzzle-piece btn-position"></i>'.$lang['extras'].'', '<i class="fa fa-address-card btn-position"></i>Список пользователей' ) );
	...
	if (isset($_REQUEST['submit'])) {
		...
		pluginsSaveConfig();
		return print_msg( 'info', 'Список пользователей', 'Настройки успешно сохранены!', 'javascript:history.go(-1)' );
